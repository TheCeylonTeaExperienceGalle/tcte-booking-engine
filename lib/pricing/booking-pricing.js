...
export function applyDiscountRules(lineItems, discountRules = []) {
  ...
  for (const rule of rules) {
    ...
    if (rule.discountType === "PERCENTAGE") {
      discountAmount = roundMoney((grossSubtotal * Number(rule.discountValue)) / 100);
    } else {
      // FIXED_AMOUNT is a currency amount off the matching combination, per combo unit.
      discountAmount = roundMoney(Number(rule.discountValue) * comboUnits);
    }
